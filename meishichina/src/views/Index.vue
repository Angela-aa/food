	...
	<div id = "index">
		<div class="logo_warp">
			<div class="container clear">
				...
				<InputSearch @handelSearch='searchRecipe'></InputSearch>
			</div>
		</div>
		<div class="main_body">
			<div class="container clear">
				<div class="banner_1 clear ">
					<div class="carrousel">
						...
						<div class="carrousel_img">
							<router-link target="_blank" :to="`/mofang/${img.mf_id}`" v-for="(img,i) in imgArr" :key="i"><img :src="img.banner" :id="img.mf_id"
								 v-show="i == mark" @mouseenter="mouseenter()" @mouseleave="mouseleave()" /></router-link>
					...
					<div class="block">
						...
						<el-carousel-item v-for="(img,i) in imgArr" :key="i">
							<!-- <h3>{{ item }}</h3> -->
						<!-- <div class="carrousel_img"> -->
							<router-link target="_blank" :to="`/mofang/${img.mf_id}`"><img :src="img.banner" :id="img.mf_id"
								 /></router-link>
						</el-carousel-item>
						</el-carousel>
					</div>
					...
					<div class="slidesContainer">
						<ul :style="{width:303*len + 'px',transform:`translate(-${activeIndex*100/len}%)`,transition:isResetIndex? '': `transform ${transitionInterval/1000}s`}">
							<li class="slide" v-for="(item,index) in msg" :key="index">
								<h2>
									<router-link target="_blank"  :to="`/details/${item.recipe_id}`">{{item.recipe_name}}</router-link>
								</h2>
								<p>
					...
					<div :class="{big_list:true,clear:true,on:clickOnState.xinxiucaipu == 1}">
						<ul>
							<li v-for="(item,index) in newlink" :key="index">
								<router-link target="_blank" :to="`/details/${item.recipe_id}`"><i><img :src="item.recipe_pic" /></i>
									<p>{{item.recipe_name}}</p>
								</router-link>
								<router-link target="_blank" :to="`/details/${item.recipe_id}`" class="user">{{item.user_name}}</router-link>
							</li>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getRecipe() {
				this.axios.get(setIP()+"index/getRecipe")
					.then(res => {
						console.log('菜谱',res);
						...
						for(let i = 0;i<res.data.length;i++){
							...
							res.data[i].recipe_pic = setIP()+ res.data[i].recipe_pic;
							// console.log(res.data[i].recipe_pic)
						}
			...
			getTopic() {
				this.axios.get(setIP()+"index/getTopic")
					.then(res => {
						console.log("话题",res)
						this.topic = res.data.topic;
						this.topic_pic = res.data.pic;
						for (var i in this.topic) {
							this.topic[i].upic = setIP()+this.topic[i].upic;
							// console.log(this.topic[i].upic)
							this.topic[i].pic = []
							for (var j in this.topic_pic)
								if (this.topic[i].tid == this.topic_pic[j].tid) {
									
									this.topic[i].pic.push(setIP()+this.topic_pic[j].tp_src);
								}
						}
						this.topic.sort(function () {
							return 0.5 - Math.random()
						})
			...
			getMofang() {
				this.axios.get(setIP()+'index/getMofang')
					.then(res => {
						console.log('魔方',res)
						for(let i=0;i<res.data.length;i++){
							res.data[i].banner = setIP() + res.data[i].banner;
						}
		...
		watch: {
			activeIndex(newActiveIndex, oldActiveIndex) {
				if ((newActiveIndex === 3 && oldActiveIndex === (this.len - 3)) || (oldActiveIndex === 0 && newActiveIndex === this
						.len - 6)) {
...
<style>
  ...
  #index .el-carousel__item:nth-child(2n) {
     background-color: transparent;
  }
  
  #index .el-carousel__item:nth-child(2n+1) {
