    ...
    <div id="main">
        ...
        <div class="search">
            <div class="search-box">
                ...
                <!-- <a href="javascript:;" title="搜索" class="" id="search">搜索</a><input type="text" id="" class=""> -->
            </div>
        </div>
        <div class="list">
    ...
    <div id="content-box">
        <div class="container">
            <div id="slider">
                <el-carousel :interval="4000" type="card" height="300px">
                    <el-carousel-item v-for="(item, index) in sliders" :key="index">
                        ...
                        <div class="os"></div>
                        <p class="number" style="width:100%;text-align:center">{{item.num}}<br><span>菜谱篇</span></p>
                    </el-carousel-item>
                </el-carousel>
            </div>
            <div class="pic-list">
            ...
            <div class="loading" id="loading-more">
                <a href="javascript:void(0);" class="" @click.stop.prevent="searchRecipe">查看更多</a> 
            </div>
        </div>
    </div>
    <Footer></Footer>
    ...
    export default{
      ...
      methods:{
                ...
                getRecipe(){
                    ...
                    if(!this.hasMore){
                        this.$message({
                        message: '暂无更多菜谱，看看其他的吧~~',
                        type: 'warning'
                        });
                    }
                    var url=setIP()+"recipe?pno="+this.pageIndex+"&pageSize="+this.pageSize;
                    this.$http.get(url).then(result=>{
                        console.log(result.body);
                        result.body.data.map(item=>{
                            item.recipe_pic = setIP() + item.recipe_pic;
                        })
        ...
        // window.location.reload();
                    ...
                    if(!this.hasMore){
                        ...
                        // this.rname = this.$route.params.rname?this.$route.params.rname:'';
                        this.rname = this.$route.query.rname?this.$route.query.rname:'';
                        // {}
                        // {rname:"鱼"}
                        var url=setIP()+"search?pno="+this.pageIndex+"&pageSize="+this.pageSize+"&rname="+this.rname;
                        this.$http.get(url).then(result=>{
                            console.log(result.body);
                            if(result.body.data.length == 0){
                                this.$message({
                                    message: '没有您要搜索的菜谱哦~~~搜搜其他的吧~~',
                                    type: 'warning'
                                });
                            }
                            result.body.data.map(item=>{
      ...
      created(){
        ...
        //   this.$route.params?this.getRecipe():searchRecipe();
        this.searchRecipe(); 
      },
      computed:{
        containerStyle() {
...
<style>
  ...
  #slider .el-carousel__item:nth-child(2n) {
     background:transparent;
  }
  
  #slider .el-carousel__item:nth-child(2n+1) {
     background:transparent;
  }
</style>
